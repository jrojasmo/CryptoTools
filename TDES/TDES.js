...
image1.src = 'Cameraman.png';              // Path de la imagen, o URL   ____INPUT____
...
function Three_keys(original_key,P48,P56,shift_table) {
    //Create K_1, K_2, K_3 arrays for TDES
    key_1 = original_key.slice(0,16);
    key_2 = original_key.slice(16,32);
    key_3 = original_key.slice(32,48);
    set_key_1 = key_generator(key_1,P48,P56,shift_table);
    set_key_2 = key_generator(key_2,P48,P56,shift_table);
    set_key_3 = key_generator(key_3,P48,P56,shift_table);
    Key_1_2_3 = [set_key_1,set_key_2,set_key_3];
    return Key_1_2_3;
}
...
function CipherDES(PlainText, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
...
function DecipherDES(CipherText, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
    decipher_text = CipherDES(CipherText, P32, Keys_t[1], IPDES, IPIDES, E_PDES, S8BOX);
    return decipher_text;
}
function Cipher_TDES(PlainText, P32, Key, IPDES, IPIDES, E_PDES, S8BOX) {
    KEY_1 = Key[0];
    KEY_2 = Key[1];
    KEY_3 = Key[2];
    out1 = CipherDES(PlainText, P32, KEY_1[0], IPDES, IPIDES, E_PDES, S8BOX);
    out2 = DecipherDES(out1, P32, KEY_2, IPDES, IPIDES, E_PDES, S8BOX);
    Cipher_text_TDES = CipherDES(out2, P32, KEY_3[0], IPDES, IPIDES, E_PDES, S8BOX);
    return(Cipher_text_TDES);
}
function Decipher_TDES(CipherText, P32, Key, IPDES, IPIDES, E_PDES, S8BOX) {
    KEY_1 = Key[0];
    KEY_2 = Key[1];
    KEY_3 = Key[2];
    out1 = DecipherDES(CipherText, P32, KEY_3, IPDES, IPIDES, E_PDES, S8BOX);
    out2 = CipherDES(out1, P32, KEY_2[0], IPDES, IPIDES, E_PDES, S8BOX);
    Decipher_text_TDES = DecipherDES(out2, P32, KEY_1, IPDES, IPIDES, E_PDES, S8BOX);
    return(Decipher_text_TDES);
}

//------------------------------------ ECB -------------------------------------------

function CipherTDES_image_ECB(scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
    sup_sd = 0;
    cont = 0;
    ...
    for (let i = 0; i <=scannedData.length-10; i+=1) {
        if (!((i+1)%4===0)){
            ...
            if (cont2===8) {
                ...
                cipher_pixels = Cipher_TDES(Pl_txt_hexa, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX);
...
function DecipherTDES_image_ECB(scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX){
    sup_sd = 0;
    cont = 0;
    let pl_txt_verification = [];
    let array_8 = [];
    ...
    for (let i = 0; i <= scannedData.length-10 ; i+=1) {
        if (!((i+1)%4===0)){
            ...
            if (cont2===8) {
                ...
                Pl_txt_hexa = bin2hex2(Pl_txt.join(''));
                cipher_pixels = Decipher_TDES(Pl_txt_hexa, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX);
                ci_pi2 = array_f(cipher_pixels);
                for (let j = 0; j < 8; j++) {
                    color_c = ci_pi2.slice(j*8,(j*8)+8);
                    color_c = color_c.join('');
...
function CipherTDES_image_CBC(IV, scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
    IV_bin = array_f(IV);
    sup_sd = 0;
    cont = 0;
    let pl_txt_verification = [];
...
function DecipherTDES_image_CBC(IV, scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX){
    IV_bin = array_f(IV);
    sup_sd = 0;
    cont = 0;
    let pl_txt_verification = [];
...
// --------------------------------- CFB -----------------------------------------------------
function CipherTDES_image_CFB(IV, scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
    //IV = array_f(IV);
    sup_sd = 0;
    cont = 0;
    let pl_txt_verification = [];
    ...
    for (let i = 0; i <=scannedData.length-10; i+=1) {
        if (!((i+1)%4===0)){
            ...
            if (cont2===8) {
                ...
                cipher_pixels = Cipher_TDES(IV, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX);
                ci_pi2 = array_f(cipher_pixels);
                ci_pi2 = xor_bin(ci_pi2,Pl_txt);
                IV = bin2hex2(ci_pi2.join(''));
                for (let j = 0; j < 8; j++) {
                    color_c = ci_pi2.slice(j*8,(j*8)+8);
                    color_c = color_c.join('');
                    color_c_d = convertNumber(color_c,2,10);
...
function DecipherTDES_image_CFB(IV, scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX){
    sup_sd = 0;
    cont = 0;
    let pl_txt_verification = [];
    let array_8 = [];
...
// --------------------------------- OFB -----------------------------------------------------
function CipherTDES_image_OFB(IV, scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
    sup_sd = 0;
    cont = 0;
    let pl_txt_verification = [];
    let array_8 = [];
...
function DecipherTDES_image_OFB(IV, scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX){
    scannedData = CipherTDES_image_OFB(IV, scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX)
    return scannedData;
}
// --------------------------------- CTR -----------------------------------------------------

function CipherTDES_image_CTR(IV, scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX) {
    sup_sd = 0;
    cont = 0;
    let pl_txt_verification = [];
    let array_8 = [];
...
function DecipherTDES_image_CTR(IV, scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX){
    scannedData = CipherTDES_image_CTR(IV, scannedData, P32, Keys_t, IPDES, IPIDES, E_PDES, S8BOX)
    return scannedData;
}
...
//     KEY     (192 BITS EN HEXADECIMAL) STR
// IV e IV_CTR (64 BITS EN HEXADECIMAL) STR
...
image1.addEventListener('load', function(){  // Imprime la imagen
    ...
    Key = (Three_keys(Key,P48,P56,shift_table));
    ...
    CipherTDES_image_ECB(scannedData, P32, Key, IPDES, IPIDES, E_PDES, S8BOX);
    //DecipherTDES_image_ECB(scannedData, P32, Key, IPDES, IPIDES, E_PDES, S8BOX);

    //CipherTDES_image_CBC(IV, scannedData, P32, Key, IPDES, IPIDES, E_PDES, S8BOX);
    //DecipherTDES_image_CBC(IV, scannedData, P32, Key, IPDES, IPIDES, E_PDES, S8BOX);

    //CipherTDES_image_CFB(IV, scannedData, P32, Key, IPDES, IPIDES, E_PDES, S8BOX);
    //DecipherTDES_image_CFB(IV, scannedData, P32, Key, IPDES, IPIDES, E_PDES, S8BOX);

    //CipherTDES_image_OFB(IV, scannedData, P32, Key, IPDES, IPIDES, E_PDES, S8BOX);
    //DecipherTDES_image_OFB(IV, scannedData, P32, Key, IPDES, IPIDES, E_PDES, S8BOX);

    //CipherTDES_image_CTR(IV_CTR, scannedData, P32, Key, IPDES, IPIDES, E_PDES, S8BOX);
    //DecipherTDES_image_CTR(IV_CTR, scannedData, P32, Key, IPDES, IPIDES, E_PDES, S8BOX);
